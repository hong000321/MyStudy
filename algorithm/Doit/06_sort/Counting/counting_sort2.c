...
void fsort(int a[], int n, int min, int max){
    int i;
    int *f = (int*)calloc(max+1-min, sizeof(int));      //1-1
    int *b = (int*)calloc(n, sizeof(int));              //3-1

    for(i=0; i<=max-min; i++) f[i]=0;               //1-1 : 0으로 초기화
    for(i=0; i<n; i++) f[a[i]-min]++;               //1-2 : 배열 a의 요소 값에 맞는 도수분포표 만들기
    for(i=1; i<=max; i++) f[i] += f[i-1];           //2-1 : 도수분포표를 누적도수분포표로 만들기
    for(i=n-1; i>=0; i--) b[--f[a[i]-min]] = a[i];  //3   : 배열 a의 위치를 배열 a에서 참고하여 배열 b에 저장하기
